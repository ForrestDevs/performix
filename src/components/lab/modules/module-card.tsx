...
import { getModuleById, getModuleCompletion } from '@/lib/data/lab'
...
export async function ModuleCard({ moduleId, hasPlan }: { moduleId: number; hasPlan: boolean }) {
  ...
  const progress = await getModuleCompletion(labModule.id)
      ...
      <CardContent className="flex flex-col space-y-4 flex-grow pt-2">
        ...
        <div className="flex items-center gap-4 text-sm text-gray-500">
          <div className="flex items-center gap-1">
            ...
            <span>{labModule.volumes?.docs?.length || 0} volumes</span>
          </div>
          <div className="flex items-center gap-1">
            <BookOpen className="h-4 w-4" />
            <span>{labModule.lessons?.docs?.length || 0} lessons</span>
          </div>
        </div>
      </CardContent>
